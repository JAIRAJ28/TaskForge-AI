...
const updateTask = async (req, res) => {
  try {
    ...
    if (columnId !== undefined) {
      ...
      if (!targetColumn) {
        return res.status(400).json({ error: true, message: "columnId does not belong to this project." });
      }
      setDoc.columnId = columnId;
    } else {
      if (title !== undefined) {
        if (typeof title !== "string" || title.trim().length < 3) {
          return res.status(400).json({ error: true, message: "title must be a string ≥ 3 chars." });
        }
        setDoc.title = title.trim();
      }
      if (description !== undefined) {
        if (typeof description !== "string") {
          return res.status(400).json({ error: true, message: "description must be a string." });
        }
        setDoc.description = description.trim();
      }
      if (difficulty !== undefined) {
        if (typeof difficulty !== "string" || !DIFFICULTIES.has(difficulty)) {
          return res.status(400).json({ error: true, message: "difficulty must be one of: easy | medium | hard." });
        }
        setDoc.difficulty = difficulty;
      }
      if (order !== undefined) {
        if (typeof order !== "number" || order < 0) {
          return res.status(400).json({ error: true, message: "order must be a non-negative number." });
    ...
    const io = getIO();
    io.to(`project:${current.projectId}`).emit("task:updated", { task: updated });
    if (setDoc.columnId !== undefined && String(setDoc.columnId) !== String(current.columnId)) {
      io.to(`project:${current.projectId}`).emit("task:moved", {
        taskId: updated._id,
        fromColumnId: current.columnId,
        toColumnId: updated.columnId,
    ...
    } else if (setDoc.order !== undefined) {
      io.to(`project:${current.projectId}`).emit("task:reordered", {
        taskId: updated._id,
        columnId: updated.columnId,
        order: updated.order,
    ...
    return res.status(200).json({ error: false, message: "Task updated successfully.", task: updated });
  } catch (err) {
